...
//            }
    ...
    private static void findPath(int[][] board, Point target, int x, int y, int turn, List<Integer> result) {
        if (x == target.x && y == target.y) {
            ...
            board[y][x] = turn;
            // move up
            findPath(board, target, x, y - 1, turn + 1, result);
            // move down
            findPath(board, target, x, y + 1, turn + 1, result);
            // move left
            findPath(board, target, x - 1, y, turn + 1, result);
            // move right
            findPath(board, target, x + 1, y, turn + 1, result);
        }
    }
